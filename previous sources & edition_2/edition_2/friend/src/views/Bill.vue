<template>
  <div>
    <div styple="padding-top: 20px;">
      <span style="font-size: 30px;color:blue;margin-top: 20px;">ORDER INFORMATION</span>
    </div>
    ...
    <el-table :data="readTableData" style="width: 100%;font-size: 15px;font-weight: bold" border height="500">
      ...
      <el-table-column prop="orderTime" label="ORDER TIME" align="center"></el-table-column>
    </el-table>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      load() { // 加载所有bill信息
        axios.get('/bill/selectAllBills').then(res => {
    ...
    computed: {
      readTableData() { // 只展示当前用户的账单信息
        ...
        return this.tableData.filter(item => {
          if (this.search == '')
            return item.userAccount.toLowerCase().includes('') && item.userAccount === account;
          else
            return item.userAccount.toLowerCase().includes(this.search.toLowerCase()) && item.userAccount === account;
        });
      }
    }
  }
